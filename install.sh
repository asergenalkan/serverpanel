...
install_packages() {
    ...
    if ! grep -r 'ondrej/php' /etc/apt/sources.list.d/ > /dev/null 2>&1; then
        ...
        # PPA ekleme - başarısız olursa hata göster
        if ! add-apt-repository -y ppa:ondrej/php > /dev/null 2>&1; then
            log_warn "PPA eklenemedi, alternatif yöntem deneniyor..."
            
            # Alternatif: GPG key'i manuel ekle
            ...
            # Sources list'e manuel ekle
            source /etc/os-release
            echo "deb https://ppa.launchpadcontent.net/ondrej/php/ubuntu ${VERSION_CODENAME} main" > /etc/apt/sources.list.d/ondrej-php.list
        fi
        
        apt-get update > /dev/null 2>&1
        
        # PPA'nın eklendiğini doğrula
        if apt-cache policy php${PHP_VERSION}-fpm | grep -q "ondrej"; then
            log_done "Ondrej PHP PPA eklendi"
        else
            log_warn "Ondrej PPA doğrulanamadı, sistem PHP kullanılacak"
        fi
    else
    ...
    if ! DEBIAN_FRONTEND=noninteractive apt-get install -y "${php_packages[@]}" > /dev/null 2>&1; then
        ...
        # Tekrar dene, bu sefer hataları göster
        if ! DEBIAN_FRONTEND=noninteractive apt-get install -y "${php_packages[@]}"; then
            log_error "PHP-FPM kurulamadı! Lütfen logları kontrol edin."
            exit 1
        fi
    fi
    
    # PHP-FPM binary kontrolü (farklı konumları kontrol et)
    local php_fpm_found=false
    if [[ -f /usr/sbin/php-fpm${PHP_VERSION} ]]; then
        php_fpm_found=true
    elif [[ -f /usr/sbin/php-fpm ]]; then
        php_fpm_found=true
    elif command -v php-fpm${PHP_VERSION} &> /dev/null; then
        php_fpm_found=true
    elif dpkg -l | grep -q "php${PHP_VERSION}-fpm"; then
        php_fpm_found=true
    fi
    ...
    if [[ "$php_fpm_found" == "true" ]]; then
        ...
        log_error "Manuel kurulum deneyin: apt-get install php${PHP_VERSION}-fpm"
        exit 1
    fi
