...
func (h *Handler) configureSSLVhost(domain, username string, cert *certInfo) error {
	...
	// Check if this domain has a Node.js application
	var nodejsAppID int64
	var nodejsPort int
	err := h.db.QueryRow(`
		SELECT id, port 
		FROM nodejs_apps 
		...
		AND status = 'running'
		LIMIT 1
	`, domain).Scan(&nodejsAppID, &nodejsPort)

	// If Node.js app exists, create Node.js vhost with SSL
	if err == nil && nodejsAppID > 0 {
		return h.configureNodejsSSLVhost(domain, nodejsPort, int(nodejsAppID), cert)
	}

	// Otherwise, create standard PHP vhost
...
// configureNodejsSSLVhost creates Node.js vhost with SSL support
func (h *Handler) configureNodejsSSLVhost(domain string, port, appID int, cert *certInfo) error {
	vhostPath := filepath.Join("/etc/apache2/sites-available", domain+".conf")

	// Create Node.js vhost with both HTTP and HTTPS
...
`, appID, domain, port, port, port, domain, domain,
		domain, cert.CertPath, cert.KeyPath, port, port, port, domain, domain)
